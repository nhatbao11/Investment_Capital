...
const PostFeed: React.FC<PostFeedProps> = ({ category, accentColor = 'blue', title }) => {
  ...
  useEffect(() => {
    const load = async () => {
      await fetchPosts({ category, status: 'published', page, limit: 12, search }) as any
    }
    load()
    ...
    try {
      ...
      if (post.pdf_url) {
        const fallbackUrl = post.pdf_url.startsWith('/uploads/') 
          ? `http://localhost:5000${post.pdf_url}` 
          : post.pdf_url;
  ...
  const ButtonBg = accentColor === 'green' ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700'
            ...
            <div className="p-4 sm:p-6">
              ...
              <div className="flex items-center justify-between text-xs sm:text-sm text-gray-500 mb-4 gap-2">
                <div className="flex items-center gap-3 sm:gap-4">
                  <div className="flex items-center gap-1"><FaUser className="h-3 w-3 sm:h-4 sm:w-4" /><span className="truncate">{featured[0]?.author_name || 'Y&T Capital'}</span></div>
              ...
              <motion.div key={post.id} initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} className="bg-white rounded-xl shadow-lg overflow-hidden">
                ...
                <div className="p-3 sm:p-4">
                  ...
                  <div className="flex items-center justify-between text-[10px] sm:text-xs text-gray-500 mb-3 gap-2">
                    <div className="flex items-center gap-3 sm:gap-4">
                      <div className="flex items-center gap-1"><FaUser className="h-3 w-3 sm:h-4 sm:w-4" /><span className="truncate">{post.author_name || 'Y&T Capital'}</span></div>
