...
const NewsletterModal: React.FC<NewsletterModalProps> = ({ isOpen, onClose, postId, postTitle }) => {
  const { user } = useAuth()
  ...
  useEffect(() => {
    if (isOpen) {
      console.log('Modal opened, fetching preview for post:', postId)
      fetchPreview()
    }
  ...
  useEffect(() => {
    console.log('NewsletterModal state:', {
      isOpen,
      postId,
      postTitle,
      token: !!getToken(),
  ...
  }, [isOpen, postId, postTitle, subscribersCount, preview])
  ...
  const fetchPreview = async () => {
    ...
    try {
      ...
      console.log('Fetching preview for post:', postId)
  ...
  // Tắt auto-update, chỉ update khi nhấn "Xem trước" hoặc lúc mở modal
  // useEffect cho phép chỉnh sửa dễ dàng hơn
            ...
            <div className="mb-4 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ví dụ: 🎉 Tin HOT - Phân tích VCB..."
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Ví dụ: Xin chào! Chúng tôi vừa phát hành báo cáo phân tích mới..."
                  rows={6}
                  className="w-full px-3 py-2 border-2 border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-900"
                />
                {customContent && (
