...
import { FaTimes, FaPaperPlane, FaUsers, FaEye } from 'react-icons/fa'
import { useNotification } from '../ui/Notification'

interface BookJourneyNewsletterModalProps {
  isOpen: boolean
  onClose: () => void
  bookId: number
  bookTitle: string
...
const BookJourneyNewsletterModal: React.FC<BookJourneyNewsletterModalProps> = ({
  isOpen,
  onClose,
  bookId,
  bookTitle
  ...
  useEffect(() => {
    if (isOpen) {
      console.log('Modal opened, fetching preview for book:', bookId)
      setCustomSubject(bookTitle)
  ...
  const fetchPreview = async () => {
    ...
    try {
      ...
      console.log('Fetching preview for book:', bookId)
      
      const res = await fetch(`${API_BASE}/bookjourney/${bookId}/preview-newsletter`, {
        method: 'POST',
        headers: {
  ...
  const handleSend = async () => {
    ...
    try {
      setSending(true)
      const API_BASE = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api/v1'
      const res = await fetch(`${API_BASE}/bookjourney/${bookId}/send-newsletter`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
          ...
          <div className="bg-white px-6 py-4">
            ...
            <div className="mb-4 p-4 bg-blue-50 rounded-lg">
              <p className="text-sm text-gray-600">
                <strong>Bài viết:</strong> {bookTitle}
              </p>
              <p className="text-sm text-gray-600">
            ...
            <div className="mb-4 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ví dụ: 🎉 Tin HOT - Hành trình sách mới..."
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Ví dụ: Xin chào! Chúng tôi vừa phát hành sách mới..."
                  rows={6}
                  className="w-full px-3 py-2 border-2 border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-900"
                />
                {customContent && (
...
export default BookJourneyNewsletterModal
