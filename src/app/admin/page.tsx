...
const AdminDashboard: React.FC = () => {
  ...
  const [feedbacksLimit] = useState(10)
  const [usersPage, setUsersPage] = useState(1)
  const [usersLimit] = useState(10)
  const [investmentPage, setInvestmentPage] = useState(1)
  const [investmentLimit] = useState(10)
  ...
  const loadInitialData = async () => {
    try {
      await Promise.all([
        fetchPosts({ page: postsPage, limit: postsLimit }),
  ...
  const handleSaveKnowledge = async (data: any) => {
    try {
      if (editingKnowledge) {
        const res = await updateKnowledge(editingKnowledge.id, data)
        if (res) alert('Cập nhật kiến thức đầu tư thành công')
      } else {
        const res = await createKnowledge(data)
        if (res) alert('Tạo kiến thức đầu tư thành công')
      }
    } catch (e: any) {
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center">
              <img src="/images/Logo01.jpg" alt="Y&T Capital" className="h-8 w-8 mr-3 rounded-full" />
          ...
          {activeTab === 'posts' && (
            <div className="p-6">
              ...
              <div className="flex items-center justify-between mt-4">
                <button
                  onClick={async () => { if (postsPage > 1) { setPostsPage(postsPage - 1); await fetchPosts({ page: postsPage - 1, limit: postsLimit }) } }}
                ...
                <button
                  onClick={async () => { setPostsPage(postsPage + 1); await fetchPosts({ page: postsPage + 1, limit: postsLimit }) }}
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {allFeedbacks.map((feedback) => (
                          <tr key={`all-${feedback.id}`}>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{feedback.name}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{feedback.company || '-'}</td>
                            <td className="px-6 py-4 text-sm text-gray-900 max-w-md truncate">{feedback.content}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{feedback.rating ? `${feedback.rating}/5` : '-'}</td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                                feedback.status === 'approved'
                                  ? 'bg-green-100 text-green-800'
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{new Date(feedback.created_at).toLocaleDateString('vi-VN')}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
