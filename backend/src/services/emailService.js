...
const sendNewPostNotification = async (recipients, post) => {
  ...
  // URL PDF bài viết (đường dẫn trực tiếp - không track)
  let pdfUrl;
  if (post.pdf_url) {
...
const sendInvestmentKnowledgeNotification = async (recipients, knowledge) => {
  ...
  // URL PDF investment knowledge (đường dẫn trực tiếp - không track)
  let pdfUrl;
...
const sendBookJourneyNotification = async (recipients, book) => {
  ...
  // URL PDF book journey (đường dẫn trực tiếp - không track)
  let pdfUrl;
  if (book.pdf_url) {
