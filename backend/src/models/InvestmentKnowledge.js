...
class InvestmentKnowledge {
  ...
  static async create(knowledgeData) {
    const { title, image_url, images, content, meaning, author_id, status = 'draft' } = knowledgeData;
    
    const sql = `
      INSERT INTO investment_knowledge (title, image_url, images, content, meaning, author_id, status)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
    
    const result = await executeQuery(sql, [title, image_url || null, JSON.stringify(images || []), content, meaning || null, author_id, status]);
  ...
  static async findAll(options = {}) {
    ...
    sql += ` ORDER BY ik.created_at DESC LIMIT ${numericLimit} OFFSET ${offset}`;
  ...
  static async update(id, updateData) {
    const allowedFields = ['title', 'image_url', 'images', 'content', 'meaning', 'status'];
    const updates = [];
    const params = [];
